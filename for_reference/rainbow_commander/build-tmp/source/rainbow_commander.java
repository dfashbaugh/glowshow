...
public class rainbow_commander extends PApplet {
...
final int INTERVAL = 100;// 100; //for heartbeat
...
public LightGroup checkLightControllers(ControlEvent theEvent) {
  ...
  for (Object o : lightGroups) {
    ...
    if (theEvent.isFrom(l.fixedLengthToggle)) {
      l.fixedLength = theEvent.value() == 1.0f;
      return l;
    }
...
class LightGroup {
  ...
  // DONT TOUCH
  public int color1 = color(255, 0, 0), color2 = color(0, 0, 255);
...
class LightGroupSettings { 
  ...
  public LightGroupSettings(int pattern, int mapping, int rate, int brightness, int color1, int color2, int frameOffset, int crossfadeDuration, boolean fixedLength) {
  ...
  public LightGroupSettings(String serialized) {
    ...
    if (data.length == 9) {
      this.frameOffset = PApplet.parseInt(data[6]);
      this.crossfadeDuration = PApplet.parseInt(data[7]);
      this.fixedLength = PApplet.parseInt(data[8]) == 1;
    } else { 
      this.frameOffset = 0;
...
public void setActiveAddr(Object[] activeAddr) {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "rainbow_commander" };
    if (passedArgs != null) {
